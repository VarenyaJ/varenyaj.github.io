...
let heroCanvas = document.getElementById("signal-canvas");
...
const setupSoundtrack = () => {
  ...
  const playSoundtrack = async () => {
    ...
    try {
      ...
      // Ignore blocked playback until the next explicit user action.
    }
...
const drawHero = (context, width, height, time) => {
  ...
  // 1. Setup 3D Perspective Projection
  const cx = width * 0.5;
  const cy = height * 0.45;
  const PERSPECTIVE = 300;
  
  const project = (x, y, z) => {
    // Offset Z to bring scene into view
    const pz = z + 15; 
    ...
    return {
      ...
      y: cy - y * 40 * scale,
      s: scale,
      visible: pz > -PERSPECTIVE * 0.95
    };
  };

  context.globalCompositeOperation = viz.composite;

  // 2. 3D Floor Grid (Waving Terrain)
  ...
  for (let r = 0; r <= gridRows; r++) {
    ...
    for (let c = 0; c <= gridCols; c++) {
      const x = (c - gridCols * 0.5) * gridStep;
      const z = (r - gridRows * 0.5) * gridStep - 8;
      const y = -1.8 + Math.sin(t * 1.2 + c * 0.4 + r * 0.3) * 0.15;
  ...
  // 3. Volumetric Waterfall (Back-to-Front)
  const wf = chamberFieldMap.waterfall;
  const streams = 12;
  for (let i = 0; i < streams; i++) {
    const offX = (i / streams - 0.5) * wf.width;
    const pTop = project(wf.x + offX, 2.5, wf.z);
    const pBottom = project(wf.x + offX, -1.8, wf.z);
    
    if (pTop.visible && pBottom.visible) {
      const grad = context.createLinearGradient(pTop.x, pTop.y, pBottom.x, pBottom.y);
      const alpha = 0.2 + Math.sin(t * 4 + i) * 0.1;
      grad.addColorStop(0, "transparent");
      grad.addColorStop(0.2, `rgba(74, 120, 255, ${alpha})`);
      grad.addColorStop(0.8, `rgba(125, 229, 141, ${alpha * 1.5})`);
      grad.addColorStop(1, "transparent");
      
      context.strokeStyle = grad;
      context.lineWidth = 3 * pTop.s;
      context.beginPath();
      context.moveTo(pTop.x, pTop.y);
      context.lineTo(pBottom.x, pBottom.y);
      context.stroke();
    }
  }
  ...
  for (let i = 0; i < 24; i++) {
    const stage = (t * 0.8 + i * 0.1) % 1.0;
    const x = (Math.sin(i * 133) * 0.5) * wf.width;
    const y = -1.8 + stage * 1.2;
    const z = wf.z + Math.cos(i * 77) * 0.5;
    const p = project(x, y, z);
    if (p.visible) {
      context.fillStyle = viz.cream;
      context.globalAlpha = (1 - stage) * 0.4 * p.s;
      context.beginPath();
      context.arc(p.x, p.y, 1.5 * p.s, 0, Math.PI * 2);
      context.fill();
    }
  }
  context.globalAlpha = 1.0;

  // 4. Atmospheric Spores
  for (let i = 0; i < 40; i++) {
    const noise = Math.sin(i * 999);
    const x = noise * 8;
    const z = Math.cos(i * 555) * 15 - 5;
    const y = Math.sin(t * 0.4 + i) * 2;
    const p = project(x, y, z);
    if (p.visible) {
      context.fillStyle = i % 2 === 0 ? viz.orangeStrong : viz.blueStrong;
      context.globalAlpha = (0.3 + Math.sin(t * 2 + i) * 0.2) * p.s;
      const size = (1.2 + Math.sin(t + i) * 0.4) * p.s;
      context.beginPath();
  ...
  // 5. Technical HUD Overlays (Top-most)
  ...
  // Corner Brackets
  const blen = 24;
  context.strokeStyle = viz.orangeStrong;
  context.beginPath();
  // TL
  context.moveTo(20, 20 + blen); context.lineTo(20, 20); context.lineTo(20 + blen, 20);
  // TR
  context.moveTo(width - 20 - blen, 20); context.lineTo(width - 20, 20); context.lineTo(width - 20, 20 + blen);
  // BL
  context.moveTo(20, height - 20 - blen); context.lineTo(20, height - 20); context.lineTo(20 + blen, height - 20);
  // BR
  context.moveTo(width - 20 - blen, height - 20); context.lineTo(width - 20, height - 20); context.lineTo(width - 20, height - 20 - blen);
  context.stroke();

  // Scanning Line
  const scanY = 20 + ((t * 0.15) % 1.0) * (height - 40);
  const scanGrad = context.createLinearGradient(0, scanY - 40, 0, scanY);
  scanGrad.addColorStop(0, "transparent");
  scanGrad.addColorStop(1, "rgba(255, 148, 71, 0.1)");
  context.fillStyle = scanGrad;
  context.fillRect(20, scanY - 40, width - 40, 40);
  
  context.strokeStyle = "rgba(255, 148, 71, 0.4)";
  context.beginPath();
  context.moveTo(20, scanY); context.lineTo(width - 20, scanY);
  context.stroke();

  // Metadata Labels
  context.fillStyle = viz.ink;
  context.font = "600 11px 'IBM Plex Mono'";
  context.fillText("TYPE // FALLBACK_VECTOR_SCAN", 36, 42);
  context.fillText(`COORD_REF // [${t.toFixed(2)}]`, 36, 56);
  
  context.textAlign = "right";
  context.fillText("STATUS // OPTIMAL", width - 36, 42);
  context.fillText("MODE // 2D_RECONSTITUTION", width - 36, 56);
  context.textAlign = "left";

  // Hex stream at bottom
  const hex = "0123456789ABCDEF";
  let hexLine = "";
  for(let i=0; i<32; i++) hexLine += hex[Math.floor(Math.sin(t * 10 + i) * 8 + 8)];
  context.globalAlpha = 0.4;
  context.fillText(`DATA_BUF // ${hexLine}`, 36, height - 36);
  context.globalAlpha = 1.0;
};
...
const scheduleWhenIdle = (callback, timeout = 1200) => {
  ...
  const id = window.setTimeout(callback, Math.min(320, timeout));
  cleanups.push(() => window.clearTimeout(id));
};

const scheduleAfterLoadIdle = (callback, timeout = 1200) => {
  scheduleAfterLoad(() => scheduleWhenIdle(callback, timeout));
};

const runOnFirstInteraction = (callback) => {
  let done = false;
  const teardown = () => {
...
scheduleAfterLoadIdle(setupSoundtrack, 900);
...
if (cursorShell && cursorRing && cursorDot && cursorLabel && window.matchMedia("(pointer: fine)").matches) {
  ...
  const scheduleCursorStop = () => {
    ...
    idleStopTimer = window.setTimeout(() => {
      if (!visible && !interactiveActive && !telemetryActive) {
        stopCursorLoop();
      }
    }, 180);
  };
  ...
  const setInteractive = (active, label = "open") => {
    ...
    if (active) {
      ensureCursorLoop();
    } else if (!visible && !telemetryActive) {
      scheduleCursorStop();
    }
  };
...
let heroRenderMode = "none";
let heroThreePending = false;
let heroThreeReady = false;
let contactThreePending = false;
let contactThreeReady = false;

const observeOnceInView = (element, callback, rootMargin = "180px") => {
  if (!element) {
    return () => {};
  }
...
const mountHeroFallback = () => {
  ...
  heroRenderMode = "2d";
  heroThreeReady = false;
};

const refreshHeroCanvas = () => {
  if (!heroCanvas?.parentNode) {
    return;
  }

  const replacement = heroCanvas.cloneNode(false);
  heroCanvas.parentNode.replaceChild(replacement, heroCanvas);
  heroCanvas = replacement;
};
...
const scheduleHeroThreePromotion = (timeout = 1400) => {
  if (!heroCanvas || !heroStage || reduceMotion || heroThreePending || heroThreeReady) {
    return;
  }

  heroThreePending = true;
  scheduleWhenIdle(() => {
    if (!heroThreeReady) {
      setupHeroThree();
    }
...
const scheduleContactThreeMount = (timeout = 1200) => {
  if (reduceMotion || contactThreePending || contactThreeReady) {
    return;
  }

  contactThreePending = true;
  scheduleWhenIdle(() => {
    if (!contactThreeReady) {
      setupContactThree();
    }
  }, timeout);
};
...
if (themeToggle) {
  themeToggle.addEventListener("click", () => {
    ...
    syncPaletteFromTheme();
    updateThemeToggleLabel();
    if (heroRenderMode === "3d") {
      setupHeroThree();
    } else if (heroRenderMode === "2d") {
      mountHeroFallback();
    }
    if (contactThreeReady) {
      setupContactThree();
...
if (heroCanvas) {
  mountHeroFallback();
  cleanups.push(observeOnceInView(heroStage || heroCanvas, () => {
    scheduleAfterLoad(() => scheduleHeroThreePromotion(1800));
  }, "220px"));
}

const contactCanvas = document.querySelector('[data-viz="contact"]');
if (contactCanvas) {
  cleanups.push(observeOnceInView(contactCanvas, () => {
    scheduleAfterLoad(() => scheduleContactThreeMount(1000));
  }, "280px"));
}
...
document.querySelectorAll("[data-viz]").forEach((canvas) => {
  ...
  if (drawer) {
    ...
    cleanups.push(observeOnceInView(canvas, () => {
      if (!canvasCleanup) {
        canvasCleanup = setupCanvas(canvas, drawer);
      }
    }, "240px"));
    cleanups.push(() => {
      if (canvasCleanup) {
